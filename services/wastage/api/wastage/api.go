...
func (s API) Register(e *echo.Echo) {
	...
	i.PUT("/ingest/:service", httpserver.AuthorizeHandler(s.TriggerIngest, api.AdminRole))
	i.GET("/usages/:id", httpserver.AuthorizeHandler(s.GetUsage, api.AdminRole))
	i.GET("/usages/accountID", httpserver.AuthorizeHandler(s.GetUsageIDByAccountID, api.AdminRole))
	i.GET("/usages/accountID/last", httpserver.AuthorizeHandler(s.GetLastUsageIDByAccountID, api.AdminRole))
	i.PUT("/usages/migrate", s.MigrateUsages)
	i.PUT("/usages/migrate/v2", s.MigrateUsagesV2)
	i.PUT("/usages/fill-rds-costs", s.FillRdsCosts)
	i.POST("/user", httpserver.AuthorizeHandler(s.CreateUser, api.AdminRole))
	i.PUT("/user/:userId", httpserver.AuthorizeHandler(s.UpdateUser, api.AdminRole))
	i.POST("/organization", httpserver.AuthorizeHandler(s.CreateOrganization, api.AdminRole))
	i.PUT("/organization/:organizationId", httpserver.AuthorizeHandler(s.UpdateOrganization, api.AdminRole))
}
