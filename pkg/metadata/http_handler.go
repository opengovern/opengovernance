...
type HttpHandlerFilter struct {
	db database.DatabaseFilter
}

func InitializeHttpHandlerFilter(
	postgresUsername string,
	postgresPassword string,
	postgresHost string,
	postgresPort string,
	...
	postgresSSLMode string,
	logger *zap.Logger,
) (*HttpHandlerFilter, error) {
	fmt.Println("Initializing http handler for list filters")
	...
	dbFilter := database.NewDatabase(orm)
	err = dbFilter.Initialize()
	if err != nil {
		return nil, err
	}
	fmt.Println("Initialized postgres database: ", postgresDb)

	return &HttpHandlerFilter{
		db: database.DatabaseFilter(dbFilter),
	}, nil
}
