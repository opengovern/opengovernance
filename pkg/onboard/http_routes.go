...
func createAzureCredential(ctx context.Context, name string, credType source.CredentialType, config api.AzureCredentialConfig) (*Credential, error) {
	...
	if credType == source.CredentialTypeManual {
...
func (h HttpHandler) postAzureCredentials(ctx echo.Context, req api.CreateCredentialRequest) error {
	...
	cred, err := createAzureCredential(ctx.Request().Context(), "", source.CredentialTypeManual, config)
	if err != nil {
		return err
	}
...
func (h HttpHandler) postAWSCredentials(ctx echo.Context, req api.CreateCredentialRequest) error {
	...
	cred, err := NewAWSCredential(name, metadata)
	if err != nil {
		return err
	}
	secretBytes, err := h.kms.Encrypt(config.AsMap(), h.keyARN)
