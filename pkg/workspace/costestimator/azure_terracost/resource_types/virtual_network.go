...
func decodeVirtualNetworkPeeringValues(request api.GetAzureVirtualNetworkRequest) virtualNetworkValues {
...
func (inst *VirtualNetwork) egressDataProcessedCostComponent(key, location string) *query.Component {
	...
	return &query.Component{
		...
		ProductFilter: &product.Filter{
			Provider: util.StringPtr(key),
			Location: util.StringPtr(virtualNetworkPeeringConvertRegion(inst.location)),
...
func (inst *VirtualNetwork) ingressDataProcessedCostComponent(key, location string) *query.Component {
	if inst.location == location {
		return &query.Component{
	...
	return &query.Component{
		...
		ProductFilter: &product.Filter{
			Provider: util.StringPtr(key),
			Location: util.StringPtr(virtualNetworkPeeringConvertRegion(location)),
			Service:  util.StringPtr("VPN Gateway"),
			AttributeFilters: []*product.AttributeFilter{
				{Key: "serviceFamily", ValueRegex: util.StringPtr("Networking")},
				{Key: "productName", ValueRegex: util.StringPtr("VPN Gateway Bandwidth")},
